	...
	public static void main(String[] args) 
{
	...
	public static String dayOfTheWeek(int date, int month, int year)
	{
		...
		int dayValue = (int) Math.abs((((date + Math.floor(2.6 * (((month + 9) % 12) + 1) - 0.2) + lastDigitsOfYear
				+ Math.floor(lastDigitsOfYear / 4) + Math.floor(firstDigitsOfYear / 4) - 2 * firstDigitsOfYear)) % 7));
